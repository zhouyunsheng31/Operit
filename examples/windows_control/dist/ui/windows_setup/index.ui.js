...
function parseConnectionPayload(raw) {
    ...
    try {
        ...
        const health = obj.health && typeof obj.health === "object" ? obj.health : null;
        const durationRaw = obj.durationMs;
        const durationMs = durationRaw == null ? "" : typeof durationRaw === "string" ? durationRaw.trim() : String(durationRaw);
        const hasSuccess = typeof obj.success === "boolean";
        return {
            success: hasSuccess ? obj.success : null,
            ...
            agentVersion: firstNonBlank(typeof obj.agentVersion === "string" ? obj.agentVersion : "", health && typeof health.version === "string" ? health.version : "", health && typeof health.agentVersion === "string" ? health.agentVersion : ""),
            durationMs,
...
function Screen(ctx) {
    ...
    const checkConnectionByTool = async () => {
        ...
        try {
            ...
            const rawPayload = typeof rawResult === "string" ? rawResult : JSON.stringify(rawResult == null ? {} : rawResult);
    ...
    const applyNewBaseUrlAndRetry = async () => {
        setErrorMessage("");
    ...
    const pasteConfigAndApply = async () => {
        ...
        try {
            ...
            await saveConfigAndActivatePackage(baseUrlValue, tokenValue, defaultShellValue, timeoutValue);
        }
        catch (error) {
    ...
    if (connectionCardState.value.status === "checking") {
        ...
        if (connectionCardState.value.status === "failed" ||
            connectionCardState.value.status === "notConfigured") {
            connectionCardChildren.push(ctx.UI.TextField({
            ...
            connectionCardChildren.push(isCheckingConnectionState.value
                ? ctx.UI.Button({
                    enabled: false,
                    fillMaxWidth: true,
