...
type AppliedConfigValue = string | number | boolean;
...
async function ensureRuntimeLoaded() {
  const runtimeJarPath = await ToolPkg.readResource(
...
function applyStringOption(config, configJson, key, setterName, target: AppliedConfig) {
...
function applyIntegerOption(config, configJson, key, setterName, target: AppliedConfig) {
  if (!hasOwn(configJson, key)) {
    return;
  }
...
function applyBooleanOption(config, configJson, key, setterName, target: AppliedConfig) {
  if (!hasOwn(configJson, key)) {
    return;
  }
...
export async function usage_advice() {
  return {
    success: true,
    packageName: "apktool",
    packageVersion: PACKAGE_VERSION,
...
export async function apktool_prepare_runtime() {
  try {
...
export async function apktool_decode(params) {
  try {
...
export async function apktool_install_framework(params) {
  try {
